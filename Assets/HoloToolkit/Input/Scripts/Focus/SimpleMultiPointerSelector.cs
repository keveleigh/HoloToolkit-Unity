...
namespace HoloToolkit.Unity.InputModule
{
    ...
    /// </summary>
    public class SimpleMultiPointerSelector : MonoBehaviour, ISourceStateHandler, IInputHandler
    {
        #region Settings
        ...
        private List<IPointingSource> currentPointers;
        ...
        void ISourceStateHandler.OnSourceLost(SourceStateEventData eventData)
        {
            ...
            {
                DisconnectPointer(inputSourcePointer);
            }
        }
        ...
        private void SetPointer(IPointingSource newPointer)
        {
            if (newPointer != null)
            {
                ...
                currentPointers.Add(newPointer);

                if (Cursor != null)
                {
                    GameObject newCursorGameObject = Instantiate(Cursor.gameObject);
                    Cursor newCursorCursor = newCursorGameObject.GetComponent<Cursor>();
                    newCursorCursor.Pointer = newPointer;
                    activeCursors.Add(newCursorCursor);
                    newCursorGameObject.SetActive(true);
                }
            }

            Debug.Assert(newPointer != null, "No Pointer Set!");
        }

        private void DisconnectPointer(IPointingSource oldPointer)
        {
            FocusManager.Instance.UnregisterPointer(oldPointer);
